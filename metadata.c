...
int dm_cache_read(struct dm_cache_metadata *cmd, dm_oblock_t lba, dm_oblock_t *pba)
{
	int r;
	...
	__le64 value;
	DMINFO("!!! %d", (int)cmd->root);
        dm_array_get_value(&cmd->info, cmd->root, 0, &value);
	DMINFO("!!! dm_cache_read %d", value);
	READ_UNLOCK(cmd);
						 ...
						 bool may_format_device)
{
	...
	cmd->sst_len = 100;
        ...
        uint64_t *x = kmalloc(sizeof(uint64_t) * 100, GFP_KERNEL);
	int r = dm_array_del(&cmd->info, cmd->root);
	r = dm_array_new(&cmd->info, &cmd->root,
			    cmd->sst_len,
			    get_lbapba, x);
	__le64 value;
	dm_array_get_value(&cmd->info, cmd->root, 0, &value);
	DMINFO("!!! %d  %d  %d", (int)cmd->root, r, value);
	return cmd;
}
